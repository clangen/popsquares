...
public class ProfileLibrary {
    ...
    private void loadDatabase() {
        OpenHelper dbHelper = new OpenHelper(mContext, "ProfileLibrary", null, DATABASE_VERSION);
    ...
    private class OpenHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        	if (oldVersion == 1) {
        		upgradeVersion1ToVersion2Profiles(db);
        	}
        }
        
        private void upgradeVersion1ToVersion2Profiles(SQLiteDatabase db) {
        	Cursor profiles = db.query(
    			TABLE_NAME,
    			new String[] { ID_COLUMN, SETTINGS_COLUMN },
    			null,
    			null,
    			null,
    			null,
        	...
        	try {
        		while (profiles.moveToNext()) {
        			long id = profiles.getLong(profiles.getColumnIndexOrThrow(ID_COLUMN));

        			try {
	        			final JSONObject json = new JSONObject(profiles.getString(
	    					profiles.getColumnIndexOrThrow(SETTINGS_COLUMN)));
	        			
	        			int size = json.getInt(mContext.getString(R.string.pref_size));
	        			json.remove(mContext.getString(R.string.pref_size));
	        			json.put(mContext.getString(R.string.pref_row_count), size);
	        			json.put(mContext.getString(R.string.pref_column_count), size);

	        	        final ContentValues values = new ContentValues();
	        	        values.put(SETTINGS_COLUMN, json.toString());
